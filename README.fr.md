...
SDK lecteur **HTTP-FLV** expérimental : vidéo **H.264 et H.265 (HEVC)**, audio **AAC**. Démultiplexage FLV maison (dont H.265 hérité, Enhanced RTMP, etc.), sortie **WebCodecs** vers Canvas / Web Audio ; vidéo optionnelle **WASM + WebGL**. Le dépôt est un **workspace pnpm** et utilise **Vite+** (`vp`) comme chaîne d’outils.
...
- Vidéo **`decodeMode`** : `auto` (après la première image WebCodecs ↔ WASM) | `webcodecs` | `wasm` (nécessite `public/wasm`)
...
**Couches** : `apps/website` → **`@live-player/core`** ; les artefacts de build [`wasm/`](wasm/) vont dans **`public/wasm/`** de l’hôte (ou `wasmScriptUrl`).
